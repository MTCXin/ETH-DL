...
model.eval()
image_size = 32
testset = dset.CIFAR10(root=args.data_root, train=False, download=True, transform=simba_utils.CIFAR_TRANSFORM)
...
if os.path.isfile(batchfile):
    ...
    while preds.ne(labels).sum() > 0:
        ...
        preds[idx], _ = simba_utils.get_preds(model, images[idx], 'cifar', batch_size=args.batch_size)

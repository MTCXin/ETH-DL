...
class SimBA:
    ...
    def normalize(self, x):
        return simba_utils.apply_normalization(x, self.dataset)
    ...
    def simba_batch(self, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        if order == 'rand':
            ...
            indices = simba_utils.diagonal_order(image_size, 3)[:max_iters]
        elif order == 'strided':
            indices = simba_utils.block_order(image_size, 3, initial_size=freq_dims, stride=stride)[:max_iters]
        else:
            indices = simba_utils.block_order(image_size, 3)[:max_iters]
        if order == 'rand':
        ...
        succs = torch.zeros(batch_size)
        queries = torch.zeros(batch_size)
        l2_norms = torch.zeros(batch_size)
        linf_norms = torch.zeros(batch_size)
        preds = self.get_preds(images_batch)
        prev_probs = self.get_probs(images_batch, labels_batch)
        remaining=preds.eq(preds)
        if pixel_attack:
            trans = lambda z: z
        else:
            trans = lambda z: simba_utils.block_idct(z, block_size=image_size, linf_bound=linf_bound)
        ...
        for k in range(max_iters):
            ...
            remaining_prev=remaining.clone()
            
            if targeted:
                remaining = preds.ne(labels_batch)
            else:
                remaining = preds.eq(labels_batch)
            changed_indices=remaining_prev.ne(remaining)
            l2_norms[changed_indices] = (perturbation.view(batch_size, -1).norm(2, 1))[changed_indices]
            linf_norms[changed_indices] = (perturbation.view(batch_size, -1).abs().max(1)[0])[changed_indices]
            probs[changed_indices] = self.get_probs((images_batch[changed_indices] + trans(self.expand_vector(x[changed_indices], expand_dims))).clamp(0, 1), labels_batch)
            # check if all images are misclassified and stop early
            if remaining.sum() == 0:
                adv = (images_batch + trans(self.expand_vector(x, expand_dims))).clamp(0, 1)
                succs = torch.ones(batch_size, max_iters - k)
                break
            ...
            if improved.sum() < remaining_indices.size(0):
                queries[remaining_indices[~improved]] += 1
            ...
            if (k + 1) % log_every == 0 or k == max_iters - 1:
                print('Iteration %d: queries = %.4f, prob = %.4f, remaining = %.4f' % (
                        k + 1, queries.mean(), probs_k.mean(), remaining.float().mean()))

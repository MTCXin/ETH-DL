...
parser.add_argument('--num_runs', type=int, default=20000, help='number of image samples')
parser.add_argument('--num_iters', type=int, default=25000, help='maximum number of iterations, 0 for unlimited') #change
parser.add_argument('--batch_size', type=int, default=200, help='batch size for parallel runs')
...
parser.add_argument('--resfile', type=str, default='result_black_simba——effnet.json', help='suffix appended to save file')
...
while preds.ne(labels).sum() > 0 or ext_imgs.sum()>0:
    ...
    for i in list(idx):
        if(routes[i] in res_dict.keys()):
            ext_imgs[i]=1
        images[i], labels[i] = testset[random.randint(0, len(testset) - 1)][0]
        routes[i] = testset[random.randint(0, len(testset) - 1)][1]
